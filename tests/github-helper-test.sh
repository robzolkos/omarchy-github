...
assert_jq '.state == "ready" and .login == "octocat" and .provider == "github" and .host == "https://github.com"' "$out" "ready state"
...
assert_jq '(.runs|length == 1) and (.failedRuns|length == 1)' "$out" "active and failed runs separated"
...
assert_jq '(.authored|length == 2) and (.authored[0].id == "octocat/hello#7") and (.authored[0].checks == "FAILURE")' "$out" "authored pull requests with check rollup"
assert_jq '(.authored[1].checks == "NONE") and (.authored[1].draft == true)' "$out" "missing rollup falls back to NONE"
assert_jq '.authoredTotal == 2' "$out" "authored pull request total reported"
...
: >"$GH_TEST_LOG"
out_archived=$(PATH="$sandbox:$PATH" "$HELPER" --scan off --include-archived true)
assert_jq '.authored|length == 2' "$out_archived" "authored pull requests survive the archived setting"
...
: >"$GH_TEST_LOG"
out_owned=$(PATH="$sandbox:$PATH" "$HELPER" --scan off)
...
: >"$GH_TEST_LOG"
out_scoped=$(PATH="$sandbox:$PATH" "$HELPER" --scan off --repository-scope wider)
assert_jq '.repositoryScope == "wider" and (.repositories|length) == 2 and ([.repositories[].nameWithOwner]|index("acme/work") != null)' "$out_scoped" "wider scope includes organization repositories"
grep -q 'ownerAffiliations:\[OWNER,ORGANIZATION_MEMBER\],' "$GH_TEST_LOG" || fail "wider scope did not reach the query"
if grep -q 'ownerAffiliations:OWNER,' "$GH_TEST_LOG"; then fail "owned affiliation used despite the wider scope"; fi
...
# Other endpoints are unstubbed here too, so warnings sort alongside theirs;
# assert presence rather than position.
assert_jq '(.warnings|length) > 0 and (.warnings|any(test("403")))' "$scoped" "Actions warnings keep the API error text"

echo "github helper tests passed"
