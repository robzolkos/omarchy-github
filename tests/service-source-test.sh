...
assert_contains 'repositoryScope: repositoryMode(),' \
  "the repository scope setting is not passed to the GitHub adapter"
...
assert_not_contains $'if (value === "" || loading || marking)' \
...
assert_contains 'githubApi.markAll(String(snapshot.boundary || ""), snapshot.boundaryIds' \
